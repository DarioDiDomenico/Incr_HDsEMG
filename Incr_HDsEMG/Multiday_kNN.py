...
def write_to_file(file_name, perm_str, overall_mean_acc, predict_time, save_folder):
    results_path = os.path.join(save_folder,file_name.split(os.sep)[1],'multiday','kNN')
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    # Program to append to text file using write() function
    with  open(os.path.join(results_path,'batch_permutation_results.txt'), "a") as file:
        file.write('Opt_par1: '+ str(None))
        file.write('\t')
        file.write('Opt_par2: '+ str(None))
        file.write('\t')
...
def main(parameters, perm):
    ...
    best_n_neigh = Data_Handler.hyperpar_selection(file_name = results_file_name, robust_opt = parameters['robust_opt'], model=parameters['ML_model'])
    print('best_n_neigh: ', best_n_neigh)
    parameters['n_neighbors'] = best_n_neigh
    perm_str_list = []
    ...
    for n_day,current_day in enumerate(perm):
        if n_day == 0:
            ...
            train_data, test_data = Data_Handler.create_dataloader_whole_test(file_name=current_day, parameters=parameters)
        ...
        for x_upd,y_upd in zip(X_te,Y_te):
            ...
            y_pred = int(model.predict([x_upd]))
        ...
        print('Mean accuracy on test of day ', n_day+1,': ' + str(np.mean(hist['accuracy']['acc_'+str(hist_cnt-1)])))

        overall_mean_acc_test = Data_Handler.update_overall_results(hist)

        if n_day == 0:
            overall_mean_acc = []
            overall_mean_acc.append([np.nan, np.nan])
    ...
    if parameters['save_results']:
        write_to_file(perm[0], perm_str, overall_mean_acc, predict_time, save_folder=parameters['save_folder'])
    ...
    for day in range(n_days):
        ...
        end_ind = 10 + day * 10
        if day == 0:
            plt.plot(x_ax[start_ind+2:end_ind],overall_mean_acc[start_ind+2:end_ind],marker='o',color=sns.color_palette('Set2')[day], label='Day '+str(day+1))
        else:
    ...
    plt.ylabel('Accuracy')
    plt.title('kNN model trained on the 20% of Day1 data')
    #plt.show()
    ...
    if parameters['save_fig']:
        ...
        if not os.path.exists(results_path):
            os.makedirs(results_path)
        plt.savefig(os.path.join(results_path,'+'.join(perm_str_list)+'.png'))
    plt.close()

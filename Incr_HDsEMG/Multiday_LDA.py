...
def write_to_file(file_name, perm_str, overall_mean_acc, predict_time, save_folder):
    results_path = os.path.join(save_folder,file_name.split(os.sep)[1],'multiday','LDA')
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    # Program to append to text file using write() function
    with  open(os.path.join(save_folder,file_name.split(os.sep)[1],'multiday','LDA','batch_permutation_results.txt'), "a") as file:
...
def main(parameters, perm):
    ...
    plt.ylabel('Accuracy')
    plt.title('LDA model trained on the 20% of Day1 data')
    #plt.show()
